...
def _mock_collection_for_layer(docs, metas):
    """Return a mock ChromaDB collection whose .get returns docs/metas in one batch,
    then empty on the next call (end of pagination).
    """
    mock_col = MagicMock()
...
# Legacy alias so old tests that still refer to the client-based helper don't
# accidentally fall through; any use of it now raises AttributeError loudly.
def _mock_chromadb_for_layer(*_args, **_kwargs):
    raise RuntimeError(
        "Legacy helper: use _mock_collection_for_layer and patch "
        "'mempalace.layers.get_collection' instead of chromadb.PersistentClient."
    )
...
def test_layer1_generates_essential_story():
    ...
    with (
        ...
        patch("mempalace.layers.get_collection", return_value=mock_col),
    ):
...
def test_layer1_with_wing_filter():
    ...
    assert "ESSENTIAL STORY" in result
    # Verify wing filter was passed to the collection's .get call
